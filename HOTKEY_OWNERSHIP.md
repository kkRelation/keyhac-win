...
| Shortcut group | Behavior | Source |
| --- | --- | --- |
| `C-A-S-7/8/9/0/-` | Toggle `U2/U3/U4/U0/U1` modifier export modes | `config.py` |
| `C-S-A-U` | Toggle all five exported modifier modes | `config.py` |
| `U1+;` | Open clipboard history list | `config.py` |
| `U0+0` / `U0+3` | Toggle / play recorded macro | `config.py` |
| `U0+C` | In PowerShell terminal copy current path; otherwise fallback copy selection | `config.py` |
| `U0+Left/Right/Up/Down` | Move active window by 10px | `config.py` |
| `U0+Space` | Open application launcher list | `extension/app_launcher.py` |
| `U0+A` | Copy selection first, then pass through `Alt+Left` | `extension/clipboard_ext.py` |
| `U0+Alt+Left/Right/Up/Down` | Move mouse cursor | `extension/mouse_ext.py` |
| `D-U0-A-Space` / `U-U0-A-Space` | Hold / release left mouse button | `extension/mouse_ext.py` |
| `U0-A-PageUp/PageDown` | Mouse wheel up / down | `extension/mouse_ext.py` |
| `g-c-p` / `g-p-p` / `g-l-c` | Insert git workflow snippets | `extension/snippet_sequences.py` |
| `U0-W` | Open window switcher list with incremental search | `extension/window_switcher.py` |
| `s-p-7` / `s-p-5` | Launch native `pwsh` / `powershell` | `extension/terminal_matrix_ext.py` |
| `r-p-7` / `r-p-5` | Launch `rio` with `pwsh` / `powershell` | `extension/terminal_matrix_ext.py` |
| `g-p-7` / `g-p-5` | Launch `ghostty` with `pwsh` / `powershell` | `extension/terminal_matrix_ext.py` |
| `f-n` in `texstudio.exe` | Insert `\\Footnote{}` wrapper | `extension/texstudio_ext.py` |
| `n-b`, `m-r`, `p-l`, `p-g`, `g-r`, `b-l` in `texstudio.exe` | Insert TeX color wrappers | `extension/texstudio_ext.py` |
| `U2-C/D/X/S/V/F` in `texstudio.exe` | Bracket copy / cut / paste helpers | `extension/texstudio_ext.py` |
...
- Recovery entrypoint is external: rerun Keyhac via `run_keyhac.ps1` (for example from Quicker), not via an in-Keyhac hotkey.
